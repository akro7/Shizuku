...
fun Project.setupCommon() {
    android {
        // compileSdk/buildTools/ndk are set globally in root build.gradle.kts subprojects block
        // compileOptions are overridden here only for library modules
        compileOptions {
        ...
        val genSrcTask = tasks.register("generate${name.capitalize(Locale.ROOT)}ObfuscatedSources") {
            doLast {
                ...
                try {
                    ...
                    exec {
                        commandLine(aapt, "compile", "-o", compileTmp, "--dir", resDir)
                        standardOutput = dummy; errorOutput = dummy
                    }
                    exec {
                        commandLine(aapt, "link", "-o", linkTmp, "-I", androidJar,
                            "--min-sdk-version", android.defaultConfig.minSdk,
                            "--target-sdk-version", android.defaultConfig.targetSdk,
                            "--manifest", stubXml, "--java", outSrcDir, compileTmp)
                        standardOutput = dummy; errorOutput = dummy
                    }
                    exec {
                        commandLine(aapt, "optimize", "-o", optTmp,
                            "--collapse-resource-names", linkTmp)
                        standardOutput = dummy; errorOutput = dummy
                    }
                    genEncryptedResources(optTmp, outSrcDir)
                } finally {
                    compileTmp.delete(); linkTmp.delete(); optTmp.delete(); stubXml.delete()
                }
            }
        }
        registerJavaGeneratingTask(genSrcTask, outSrcDir)
